...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def collate_fn(self, batches):
        (common_user, item, rating,
         user2categories_ids_src, user2categories_count_src, user2brands_ids_src, user2brands_count_src, user2items_ids_src, user2items_rating_src, user2aspects_ids_src, user2aspects_count_src,
         item2categories_ids_tgt, item2brands_ids_tgt, item2aspects_ids_tgt, item2aspects_polarity_tgt, item2rating_tgt) = [i for i in zip(*batches)]
        ...
        item2rating_tgt = torch.tensor(item2rating_tgt, dtype=torch.float)
        ...
        return (common_user, item, rating,
                ...
                item2categories_ids_tgt, item2brands_ids_tgt, item2aspects_ids_tgt, item2aspects_polarity_tgt, item2rating_tgt)
    ...
    def __getitem__(self, index):
        if index not in self.cache:
            ...
            user2items_src, user2items_rating_src = self.args.processed_user2items_src[user_id]
            user2items_ids_src = [self.args.idict_s[i] for i in user2items_src]
            ...
            item2aspects_tgt, item2aspects_polarity_tgt = self.args.processed_item2aspects_tgt.get(item_id, ([], []))
            item2aspects_ids_tgt = [self.args.aspect_vocab_tgt[i] for i in item2aspects_tgt]
            item2rating_tgt = self.args.processed_item2ratings_tgt[item_id]
