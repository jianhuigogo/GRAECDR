...
class DatasetHandler(object):
    ...
    def get_user2categories_brands(self, user2items, users, meta_path, idict):
        ...
        for user in users:
            ...
            for category in current_user2categories_pairs[0]:
                if category not in category_vocab:
                    category_vocab[category] = len(category_vocab)

            for brand in current_user2brand_pairs[0]:
                if brand not in brand_vocab:
                    brand_vocab[brand] = len(brand_vocab)
            user2categories[user] = current_user2categories_pairs
            user2barnds[user] = current_user2brand_pairs

        return user2categories, user2barnds, category_vocab, brand_vocab
    ...
    def get_user2aspects(self, user2aspects, users):

        def add_aspect(aspect, current_user2aspects):
        ...
        for user in users:
                    ...
                    for aspect in review_aspect:
                        if not isinstance(aspect, tuple):
                            if 'aspect' in aspect:
                                ...
                                add_aspect(aspect, current_user2aspects)
                            else:  # aspect name as key
                                for key in aspect:
                                    add_aspect(key, current_user2aspects)
                        else:
                            for inner_aspect in aspect:
            ...
            # construct vocab
            for aspect in current_user2aspects_pair[0]:
                if aspect not in aspect_vacab:
                    aspect_vacab[aspect] = len(aspect_vacab)
        return new_user2aspects, aspect_vacab
